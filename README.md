...
## Building the SDK ##
### Everlaw ###
In the `servers` repository, the root-level `pom.xml` should have sharefile-api listed as a dependency with the current version. It should also be listed in the `pom.xml` for `web`, without version. On build, it will look for the artifact in your local maven repository, or in github packages if it doesn't find it locally.

Note `build.gradle` for this project has a section `uploadArchives` for generating those artifacts. Since we're no longer hosting these packages on EFS, there's now only a single repository listed, your local maven repository (which should point at `~/.m2`). After building and deploying your new version to your local maven (`./gradlew build && ./gradlew uploadArchives`), you can test against it by updating the `pom.xml` file described above to point to your new version. When ready to deploy widely, you can publish the new version using the process described here https://everlaw.atlassian.net/wiki/spaces/ENG/pages/403963963/Github+Packages#Uploading-artifacts
